...
- UI:
  ...
  - (We also still have `LocationListView(locations:)` as a simple reusable list view.)
...
- Introduce a repository *protocol* (`InventoryRepository`) so we can swap in persistence later.
