...
final class DiskInventoryRepository {
    ...
    public func save(hotspots: [Hotspot]) {
        do {
            // Pack both lists into the backup struct
            let backup = InventoryBackup(locations: locations, items: items, hotspots: hotspots)
            let data = try JSONEncoder().encode(backup)
            try data.write(to: fileURL)
            print("💾 Saved \(locations.count) locs, \(items.count) items, and \(hotspots.count) hotspots.")
        } catch {
            print("❌ Failed to save: \(error)")
        }
    }
    
    private func load() -> (locations: [Location], items: [Item], hotspots: [Hotspot]) {
        ...
        guard let data = try? Data(contentsOf: fileURL) else {
            ...
            return (locations: [], items: [], hotspots: [])
        }

        do {
            // Try to decode the new format (Locations + Items + Hotspots)
            let backup = try JSONDecoder().decode(InventoryBackup.self, from: data)
            let loadedHotspots = backup.hotspots ?? []
            print("📂 Loaded \(backup.locations.count) locs, \(backup.items.count) items, and \(loadedHotspots.count) hotspots.")
            return (locations: backup.locations, items: backup.items, hotspots: loadedHotspots)
        } catch {
            ...
            if let oldLocations = try? JSONDecoder().decode([Location].self, from: data) {
                ...
                let migratedHotspots: [Hotspot] = []
                save(hotspots: migratedHotspots) // Upgrade file format immediately
                return (locations: oldLocations, items: [], hotspots: migratedHotspots)
            } else {
